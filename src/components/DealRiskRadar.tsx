  ...
  if (riskReps.length === 0) {
    return (
      <div className="flex items-center justify-center py-24">
        <p className="text-gray-500">No open pre-billing deals found this week.</p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3 mb-2">
        ...
        <span className="text-gray-600 text-xs">
          (Critical skills: Opening, Pain discovery, Connecting pain, Urgency, Close attempt, Value, Commitment, Call quality)
        </span>
      </div>
        ...
        {riskReps.map(
          ({ rep, criticalAvg, deltaFromTeam, riskLevel, weakestSkills, openDeals, oldDeals }) => {
            ...
            return (
              <div
                key={rep.id}
                className={`bg-gray-900 border-2 ${borderClass} rounded-lg p-5`}
              >
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-white font-semibold">{rep.repName}</h3>
                ...
                <div className="grid grid-cols-3 gap-3 mb-4 text-center">
                  <div>
                    <p className="text-xs text-gray-500 mb-0.5">Critical avg</p>
                    <p className="text-white font-medium">{Math.round(criticalAvg)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 mb-0.5">vs team</p>
                    <p
                      className={`font-medium ${deltaFromTeam < 0 ? 'text-red-400' : 'text-green-400'}`}
                    >
                      {deltaFromTeam > 0 ? '+' : ''}
                  ...
                  <div>
                    <p className="text-xs text-gray-500 mb-0.5">Open deals</p>
                    <p className="text-white font-medium">{openDeals.length}</p>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <ul className="space-y-1">
                      {weakestSkills.map((skill) => (
                        <li key={skill} className="text-xs text-gray-300 flex items-start gap-1.5">
